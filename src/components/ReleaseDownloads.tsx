...
export default function ReleaseDownloads() {
  ...
  useEffect(() => {
    ...
    fetchLatestRelease().then((release) => {
      if (cancelled) return;
      setState(release ? { status: "success", release } : { status: "empty" });
    });
    return () => {
      cancelled = true;
    };
  ...
  if (state.status === "loading") {
    return (
      <div aria-live="polite" aria-busy="true" className="flex flex-col items-center gap-3">
  ...
  if (state.status === "empty") {
    return (
      <>
        <p className="mb-12 text-center text-muted">{t("fallbackSubtitle")}</p>
  ...
  const macAsset = findAsset(release.assets, ".dmg");
  const winAsset = findAsset(release.assets, ".exe", ".msix");
  const linuxAsset = findAsset(release.assets, ".deb", ".rpm");

  return (
    <>
      <div className="mb-12 flex flex-col items-center gap-1 text-center">
